...
function Currency() {
  ...
  return (
    ...
    <div className="sm:w-[55rem] ">
      <h1 id="first" className='text-5xl font-extrabold'>Api Currency Exchange Rate</h1>
      ...
      <p> ApiCurrency is simple api to check currency rate and give you the right exchange price on short time.</p>
      <h1 className="text-2xl font-mono font-bold">Why Should I Use This?</h1>
      <p>For simple reason to use ApiCurrency is it because ApiCurrency make tings easier and get data in a simple way without the need of bigger effort .</p>
    ...
    "amount":100
      ...
      <p><span className="code">"currency_base"</span> Respresent the currency you want to exchnage whit as base like <span className="code">"USD"</span> . <br></br> <span className="code">"currency_target"</span> it's currency target you want to get exchange of like <span className="code">"EUR"</span>. <br></br> <span className="code">"amount"</span> and lastly this represent how much of much of amout to exchange whit. </p>
...
r = requests.get(url = 'http://localhost:8000/api/apicurrency/',data=DATA
                                        ,headers=HEADERS)
data = r.json()`}
            showLineNumbers={true}
            theme={dracula}
...
const apiCurrency = () => {
  axios
    .get('http://localhost:8000/api/apicurrency/', config, formData)
    .then((response) => {
